...
class MainMenu(control.State):
    def __init__(self):
        ...
        self.background_x_mult = const.DEFAULT_BACKGROUND_X_MULT
        self.background_y_mult = const.DEFAULT_BACKGROUND_Y_MULT
        self.menu_y = const.STARTING_MENU_Y
    ...
    def re_setup_menu(self):
        x = setup.screen_size.get_width()/2 - const.MENU_WIDTH/2
        y = self.menu_y
        menu_separation = const.MENU_SELECTION_OFFSET
    ...
    def set_next_state(self) -> const.MainState:
        return const.MainState.arena


    def update(self, surface: pg.Surface, current_time: float, dt: float) -> None:
    ...
    def update_sizes(self) -> None:
        ...
        if setup.screen_size.changed():
            self.re_setup_menu()
            self.background_x_mult = setup.screen_size.get_width() * const.BACKGROUND_X_SCALER
            self.background_y_mult = setup.screen_size.get_height() * const.BACKGROUND_Y_SCALER
